...
def run_e2e(n_classes, model_type, ratio_estimator='logistic', n_train=20000, n_calibrate=3000, n_validate=10000):
    ...
    x, y = sklearn.datasets.make_classification(n_samples=total_samples, n_features=40, n_informative=10, class_sep=0.65, flip_y=0.25, n_classes=n_classes)
    ...
    # Fit the calibrator
    calibrator = transport_calibration.TransportCalibration(y_calibrate_predicted, y_calibrate, training_class_probability, ratio_estimator=ratio_estimator)
    ...
    if n_classes == 2:
        ...
        alt_objs.append(transport_calibration.TransportCalibration(y_calibrate_predicted[:,1].flatten(), y_calibrate, training_class_probability, ratio_estimator=ratio_estimator))

        # Test input shape of (N,) and scalar training_class_probability
        alt_objs.append(transport_calibration.TransportCalibration(y_calibrate_predicted[:,1].flatten(), y_calibrate, float(training_class_probability[1]), ratio_estimator=ratio_estimator))

        # Test input shape of (N,C) and scalar training_class_probability
        alt_objs.append(transport_calibration.TransportCalibration(y_calibrate_predicted, y_calibrate, float(training_class_probability[1]), ratio_estimator=ratio_estimator))

        # Test input shape of (N,C) and (1,) training_class_probability
        alt_objs.append(transport_calibration.TransportCalibration(y_calibrate_predicted, y_calibrate, numpy.asarray([float(training_class_probability[1])]), ratio_estimator=ratio_estimator))
    ...
    target_domain_data = stratified_sample_on_label(class_probability, x_validate, y_validate, n_validate//4)
    ...
    y_validate_predicted_calibrated = calibrator.calibrated_probability(y_validate_predicted, class_probability)
    ...
    if n_classes == 2:
        ...
        for cal_obj in [calibrator]+alt_objs:
            ...
            alt_outputs.append(cal_obj.calibrated_probability(y_validate_predicted, class_probability))

            # Test input shape of (N,C) with scalar class_probability
            alt_outputs.append(cal_obj.calibrated_probability(y_validate_predicted, float(class_probability[1])))

            # Test input shape of (N,)
            alt_outputs.append(cal_obj.calibrated_probability(y_validate_predicted[:,1].flatten(), class_probability))

            # Test input shape of (N,) with scalar class probability
            alt_outputs.append(cal_obj.calibrated_probability(y_validate_predicted[:,1].flatten(), float(class_probability[1])))
    ...
    validation_metrics['predicted_prevalence_calibrated'] = y_validate_predicted_calibrated.mean(axis=0)
...
def stratified_sample_on_label(class_probability, x, y, n_samples):
    ...
    nclassex = numpy.asarray(numpy.round(numpy.asarray(class_probability)*n_samples), dtype=int)
    ...
    for c in range(len(class_probability)):
        ...
        try:
            ...
            raise ValueError(f'Not enough examples of class {c} to extract a subset of size {n_samples}')
