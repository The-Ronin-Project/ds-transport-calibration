...
def run_xgb_wrapper_with_shap(ratio_estimator='logistic', n_train=20000, n_calibrate=3000, n_validate=10000, make_pdfs=False):
    ...
    # Fit the calibrator
    model.transport_calibration_fit(x_calibrate, y_calibrate, training_class_probability=training_class_probability, ratio_estimator=ratio_estimator)
    ...
    for i in range(n_resamples):
        ...
        target_domain_data.append(transport_calibration_testing_utils.stratified_sample_on_label(class_probability[i], x_validate, y_validate, n_validate//4))

        # Compute uncalibrated model scores on the resampled dataset
        y_validate_predicted.append(model.transport_calibration_predict_proba_uncalibrated(target_domain_data[i]['x']))
        ...
        y_validate_predicted_calibrated.append(model.predict_proba(target_domain_data[i]['x']))

        # Compute some validation metrics
        validation_metrics[f'class_probability_{i}'] = class_probability[i]
        validation_metrics[f'predicted_prevalence_{i}'] = y_validate_predicted[i].mean(axis=0)
        validation_metrics[f'predicted_prevalence_calibrated_{i}'] = y_validate_predicted_calibrated[i].mean(axis=0)
    ...
    for i in range(n_resamples):
        ...
        shaps = (tree_explainer(target_domain_data[i]['x'][0:3]), perm_explainer(target_domain_data[i]['x'][0:3]))
        ...
        validation_metrics[f'sanitycheck_{sanity_check_count}'] = numpy.isclose(fx_shap_unc, fx_dir_unc).all()
        sanity_check_count = sanity_check_count + 1
        validation_metrics[f'sanitycheck_{sanity_check_count}'] = numpy.isclose(fx_shap_cal, fx_dir_cal).all()
        sanity_check_count = sanity_check_count + 1
        ...
        if make_pdfs:
            for j in range(3):
                ...
                for k in range(2):
                    ...
                    plt.title('{a}: calibrated pred={pred:.2f}, uncal={unc:.2f}'.format(a=algos[k], pred=y_validate_predicted_calibrated[i][j,1], unc=y_validate_predicted[i][j,1]))
