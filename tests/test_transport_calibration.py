...
class TestTransportCalibration(unittest.TestCase):
    def test_output_shape_binary_histogram(self):
        ...
        results, calibrator = transport_calibration_testing_utils.run_e2e(2, 'logistic', ratio_estimator='histogram')

        # Check that (N,2) shaped input produces correct output shape
        res1 = calibrator.calibrated_probability(numpy.asarray([0.65,0.35]*3).reshape(3,2), 0.75)
    ...
    def test_output_shape_binary_logistic(self):
        # Simulate data and train a calibrator
        results, calibrator = transport_calibration_testing_utils.run_e2e(2, 'logistic', ratio_estimator='logistic')

        # Check that (N,2) shaped input produces correct output shape
        res1 = calibrator.calibrated_probability(numpy.asarray([0.6,0.4]*3).reshape(3,2), 0.7)
        self.assertTrue(res1.shape == (3,2))
    ...
    def test_output_shape_multiclass(self):
        ...
        res1 = calibrator.calibrated_probability(numpy.asarray([0.8,0.1,0.1,0.3,0.3,0.4]).reshape(2,3), numpy.asarray([0.4, 0.4, 0.2]))
        self.assertTrue(res1.shape == (2,3))

        # Check that (C,) shaped input produces correct output shape
        res2 = calibrator.calibrated_probability(numpy.asarray([0.3,0.3,0.4]), numpy.asarray([0.4, 0.4, 0.2]))
        self.assertTrue(res2.shape == (3,))
    ...
    def test_e2e_binary_histogram(self):
        ...
        self.assertTrue(numpy.isclose(results['class_probability'], results['predicted_prevalence_calibrated'], atol=0.1).all())
