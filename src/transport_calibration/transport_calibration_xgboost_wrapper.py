...
class TransportCalibration_XGBClassifier(xgboost.XGBClassifier):
    ...
    def transport_calibration_class_probability(self, class_probability):
        ...
        if self._transport_calibration_calibrator is None:
            ...
            if class_probability is None:
                class_probability = self._transport_calibration_calibrator.training_class_probability

            # Store the set value, converting to a numpy array and renormalizing it if necessary
            self._transport_calibration_class_probability = self._transport_calibration_calibrator._repair_class_probability(numpy.asarray(class_probability).flatten())


    def transport_calibration_fit(self, training_features, training_labels, training_class_probability=None, ratio_estimator=None):
        ...
        if self._transport_calibration_calibrator is not None:
            print('Warning- calibrator is already trained- now retraining and overwriting')

        # Determine the number of classes from the parent class
        try:
            n_classes = self.n_classes_
        except AttributeError:
            raise ValueError('Does not seem that this classifier was trained yet- need to train it first.')
        ...
        if isinstance(training_labels, numpy.ndarray):
            ...
            if min(training_labels) < 0 or max(training_labels) >= n_classes:
                raise ValueError('Invalid range of training labels: must be from 0 to n_classes-1')
        else:
            raise ValueError('Invalid input type for training_labels: must be a numpy array')
        ...
        training_scores = self.transport_calibration_predict_proba_uncalibrated(training_features)

        # Instantiate and train the calibrator
        self._transport_calibration_calibrator = transport_calibration.TransportCalibration(training_scores, training_labels, training_class_probability, ratio_estimator=ratio_estimator)
    ...
    def predict_proba(self, features, class_probability=None):
        ...
        if self._transport_calibration_calibrator is None:
            raise ValueError('Need to first fit the calibrator by calling transport_calibration_fit')
        if self.transport_calibration_class_probability is None:
            raise ValueError('Need to set the value of transport_calibration_class_probability for the target domain')
        ...
        return self._transport_calibration_calibrator.calibrated_probability(scores, self.transport_calibration_class_probability)